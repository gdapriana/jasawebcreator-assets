...
class Xview
{
	...
	public function __construct()
	{
		...
		$this->siteOption = $this->get_siteOption();
		
	}

	function get_siteOption(){
	...
	public function get_addon_content($product_id='', $type='')
	{
		...
		if ($type == 'blog') {
			$addon_contents = $this->ci->db->query('SELECT * FROM addon_contents WHERE company_id="'.$company_id.'" AND product_id="'.@$product_id.'" AND type="BLOG"')->result();
		}
		if ($type == 'page') {
			$addon_contents = $this->ci->db->query('SELECT * FROM addon_contents WHERE company_id="'.$company_id.'" AND product_id="'.@$product_id.'" AND type="PAGE"')->result();
		}
	...
	{
		$company_id = COMPANY_ID;
		$Images = $this->ci->db->query('SELECT * FROM images WHERE company_id="'.$company_id.'" AND id="'.@$id.'"')->result();
		$result = json_decode(json_encode($Images));
		return $result;
	}
	...
	public function components($collection_id = '', $dev='0')
	{
		$data = @$GLOBALS['API_RESULT'];
       ...
       $collection_page_id = @$collection_id;
        ...
        foreach ($setting as $item) {
            ...
            if ($item->component == 'product') {
                if($item->criteria == '' || empty($item->criteria)){
					...
					if($this->siteOption['sorting_list_supported'] == 1){
						$result[$nu] = array(
							'setting' => $item,
							'file' => $item->component,
							'data' => @$this->ci->db->query('SELECT * FROM products WHERE company_id="'.$company_id.'" AND draft="0" ORDER BY sort DESC, updated_at DESC LIMIT '.$item->number_post)->result(),
						);
					}else{
                    $result[$nu] = array(
                        'setting' => $item,
                        'file' => $item->component,
						// ambil data yang terhubung di table addon_contents
					...
					if($this->siteOption['sorting_list_supported'] == 1){
						$result[$nu] = array(
							'setting' => $item,
							'file' => $item->component,
							'data' => @$this->ci->db->query('SELECT * FROM products WHERE tags LIKE "%'.$item->criteria.'%" AND company_id="'.$company_id.'" AND draft="0" ORDER BY sort DESC, updated_at DESC LIMIT '.$item->number_post)->result(),
						);
					}else{
						$result[$nu] = array(
							'setting' => $item,
                ...
                if($item->criteria == '' || empty($item->criteria)){
					...
					if($this->siteOption['sorting_list_supported'] == 1){
						$result[$nu] = array(
							'setting' => $item,
							'file' => $item->component,
							'data' => @$this->ci->db->query('SELECT * FROM blog_post WHERE company_id="'.$company_id.'" AND draft="0" ORDER BY sort DESC, updated_at DESC LIMIT '.$item->number_post)->result(),
						);
					}else{
						$result[$nu] = array(
							'setting' => $item,
					...
					if($this->siteOption['sorting_list_supported'] == 1){
						$result[$nu] = array(
							'setting' => $item,
							'file' => $item->component,
							'data' => @$this->ci->db->query('SELECT * FROM blog_post WHERE tags LIKE "%'.$item->criteria.'%" AND company_id="'.$company_id.'" AND draft="0" ORDER BY sort DESC, updated_at DESC LIMIT '.$item->number_post)->result(),
						);
					}else{
						$result[$nu] = array(
							'setting' => $item,
            ...
            if ($item->component == 'page') {
                $result[$nu] = array(
                    'setting' => $item,
                    'file' => $item->component,
                    'data' => @$this->ci->db->query('SELECT * FROM pages WHERE company_id="'.$company_id.'" AND id="'.$item->page_id.'"')->result(),
					'addon_contents' => @$this->get_addon_content($item->page_id,'page'),
                );
            }else{
                // Custom
                $result[$nu] = array(
		...
		// if $dev == 1 then return the result
		if($dev == 1){
			return $result;
		}
	...
	public function sidebar_options($type = '', $item='', $company_id='',$related='')
    {
		...
		/*List Sorting Post*/
        if ($item->data_widget == 'sorting-post') {
			if($type == 'blog' && $type == 'product'){
            	$data .='ORDER BY sort DESC , updated_at DESC ';
			}else{
	...
	public function get_review($product_id='')
	{
		if($product_id == ''){
			$data = $GLOBALS['API_RESULT'];
			$company_id = COMPANY_ID;
			$review = $this->ci->db->query('SELECT * FROM reviews WHERE company_id="'.$company_id.'" ORDER BY id DESC')->result();
			return $review;
		}else{
			$data = $GLOBALS['API_RESULT'];
			$company_id = COMPANY_ID;
			$review = $this->ci->db->query('SELECT * FROM reviews WHERE company_id="'.$company_id.'" AND product_id="'.$product_id.'" ORDER BY id DESC')->result();
			return $review;
		}
