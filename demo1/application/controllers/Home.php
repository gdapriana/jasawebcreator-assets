...
class Home extends MY_API
{
        ...
        if(!$this->ifNotFound_single($data)){
            ...
            if($this->input->get('dev') == 1){
                ...
                ->set_output(json_encode([
                    'data' => $data,
                    'component' => @$this->xview->components('',1),
                    'text' => 'Success 200',
                    'type' => 'success'
                ]));
            }else{
    ...
    public function blog_post($action = null)
    {
        ...
        //   Addon Contents
          $addon_contents = $this->db->query('SELECT * FROM addon_contents WHERE company_id="'.$company_id.'" AND product_id="'.@$result[0]->id.'" AND type="BLOG"')->result();
  ...
  public function test_cat($slug = null)
    ...
    public function page($action = null)
    {
        $company_id = $this->company_id;
          ...
          $addon_contents = $this->db->query('SELECT * FROM addon_contents WHERE company_id="'.$company_id.'" AND product_id="'.@$page[0]->id.'" AND type="PAGE"')->result();
